<?php
// Dynamic CRUD UI for students-like tables.
$candidates = ['students','student','learners','pupils'];
...
if(isset($_GET['debug'])){
  ...
  if($tbl){
    ...
    // show a sample of tables in DB so user can see naming differences
    $list = [];
...
if(!$tbl){ ?>
  <div class="card mb-4"><div class="card-body">
    <h3 class="card-title">Học viên</h3>
    <p class="card-text">Chưa tìm thấy bảng học viên trong DB. Tạo bảng `students` để sử dụng giao diện quản lý.</p>
  </div></div>
<?php } else {
...
<div class="card mb-4">
  <div class="card-body">
    <div class="d-flex justify-content-between align-items-center mb-3">
      <h3 class="card-title mb-0">Quản lý Học viên (<?= htmlspecialchars($tbl) ?>)</h3>
      <div>
        <button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#addItemModal">Thêm học viên</button>
      </div>
    </div>
        ...
        <tbody>
          <?php foreach($rows as $r): ?><tr>
            ...
            <td>
              ...
              <form method="post" action="index.php?page=students" class="d-inline" onsubmit="return confirm('Bạn có chắc muốn xóa?');">
...
<div class="modal fade" id="addItemModal" tabindex="-1"><div class="modal-dialog modal-dialog-centered"><div class="modal-content">
  <form method="post" action="index.php?page=students">
    ...
    <input type="hidden" name="csrf_token" value="<?= htmlspecialchars(csrf_token()) ?>">
    <div class="modal-header"><h5 class="modal-title">Thêm học viên</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
    <div class="modal-body">
      <?php foreach($cols as $c){ if(strpos($c['Extra'],'auto_increment')!==false) continue; $name=$c['Field']; $type='text'; if(stripos($name,'date')!==false) $type='date'; elseif(stripos($name,'time')!==false) $type='datetime-local'; elseif(stripos($c['Type'],'int')!==false) $type='number';
        // Render select for class_id if present
        if($name === 'class_id'){
          echo '<div class="mb-2"><label class="form-label">Lớp</label>';
          echo '<select name="class_id" class="form-select">';
          $cres = @mysqli_query($conn, "SELECT id, name FROM `classes` ORDER BY id ASC");
          if($cres){ while($cr = mysqli_fetch_assoc($cres)){ echo '<option value="'.htmlspecialchars($cr['id']).'">'.htmlspecialchars($cr['name']).'</option>'; } }
          echo '</select></div>';
          continue;
        }
      ?>
...
<div class="modal fade" id="editItemModal" tabindex="-1"><div class="modal-dialog modal-dialog-centered"><div class="modal-content">
  <form method="post" action="index.php?page=students">
    ...
    <div class="modal-header"><h5 class="modal-title">Sửa học viên</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
    <div class="modal-body">
      <?php foreach($cols as $c){ if($c['Field']===get_primary_key_from_cols($cols)) continue; $name=$c['Field']; $type='text'; if(stripos($name,'date')!==false) $type='date'; elseif(stripos($name,'time')!==false) $type='datetime-local'; elseif(stripos($c['Type'],'int')!==false) $type='number';
        if($name === 'class_id'){
          echo '<div class="mb-2"><label class="form-label">Lớp</label>';
          echo '<select name="class_id" id="edit-class_id" class="form-select">';
          $cres = @mysqli_query($conn, "SELECT id, name FROM `classes` ORDER BY id ASC");
          if($cres){ while($cr = mysqli_fetch_assoc($cres)){ echo '<option value="'.htmlspecialchars($cr['id']).'">'.htmlspecialchars($cr['name']).'</option>'; } }
          echo '</select></div>';
          continue;
